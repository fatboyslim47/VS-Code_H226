...
BUDGET_WORKBOOK_NAME = 'R&D_budget_2026.xlsx'
ENGINEERING_HOURLY_RATE = 81.0
...
def render_qmd_report(script_dir: Path, output_prefix: str, executed_at: str) -> bool:
    ...
    qmd_file = script_dir / 'CleanEnergy_LP_Project-plotting.qmd'
...
def load_budget_hours_by_project(script_dir: Path) -> dict[int, dict[str, float]]:
    """Load monthly project budgets from the workbook and convert dollars to hours."""
    budget_file = script_dir / BUDGET_WORKBOOK_NAME
    if not budget_file.exists():
        print(f'Budget workbook not found; NPD budget lines will be skipped: {budget_file}')
        return {}

    budget_df = pd.read_excel(budget_file)
    if 'Project Id' not in budget_df.columns:
        print('Budget workbook has no Project Id column; NPD budget lines will be skipped.')
        return {}

    monthly_budget_columns = {
        calendar.month_name[month_number]: str(month_number)
        for month_number in range(1, 13)
        if calendar.month_name[month_number] in budget_df.columns
    }
    if not monthly_budget_columns:
        print('Budget workbook has no monthly budget columns; NPD budget lines will be skipped.')
        return {}

    budget_hours_by_project: dict[int, dict[str, float]] = {}
    for _, budget_row in budget_df.iterrows():
        project_id = pd.to_numeric(budget_row['Project Id'], errors='coerce')
        if pd.isna(project_id):
            continue
        budget_hours_by_project[int(project_id)] = {
            month_key: float(pd.to_numeric(budget_row[column_name], errors='coerce') or 0) / ENGINEERING_HOURLY_RATE
            for column_name, month_key in monthly_budget_columns.items()
        }

    print(f'Loaded monthly budgets for {len(budget_hours_by_project)} project(s) from {budget_file.name}')
    return budget_hours_by_project
...
def create_entity_employee_sparkline_plots(
    ...
    script_dir: Path,
    latest_revision: int,
    budget_hours_by_project: dict[int, dict[str, float]] | None = None,
) -> list[tuple[str, Any]]:
    ...
    for entity_id in entity_ids:
        ...
        is_npd_project = (
            ...
            and PROJECT_CATEGORY_BY_ID.get(entity_id) == 'NPD'
            and budget_hours_by_project is not None
        )
        budget_series = None
        if is_npd_project:
            project_budget = budget_hours_by_project.get(entity_id, {})
            budget_series = pd.Series(
                [project_budget.get(str(month), 0.0) for month in month_labels],
                index=month_labels,
                dtype=float,
            )
        cumulative_series = total_series.cumsum()
        cumulative_budget_series = budget_series.cumsum() if budget_series is not None else None
        cumulative_max_hours = max(
            float(cumulative_series.max()),
            float(cumulative_budget_series.max()) if cumulative_budget_series is not None else 0.0,
            1.0,
        )
        cumulative_ax.plot(
        ...
        if cumulative_budget_series is not None:
            cumulative_ax.plot(
                range(len(month_labels)),
                cumulative_budget_series.values,
                color='#e76f51',
                linestyle='--',
                linewidth=2,
                label='Budget',
            )
            cumulative_ax.legend(loc='upper left', fontsize=7, frameon=False)
        ...
        total_max_hours = max(
            float(total_series.max()),
            float(budget_series.max()) if budget_series is not None else 0.0,
            1.0,
        )
        if budget_series is not None:
            total_ax.plot(
                range(len(month_labels)),
                budget_series.values,
                color='#e76f51',
                linestyle='--',
                linewidth=2,
                label='Budget',
            )
            total_ax.legend(loc='upper left', fontsize=7, frameon=False)
...
def main() -> None:
    ...
    file_prefix = f'CleanEnergy_LP_Project_{execution_time.strftime("%Y-%m-%d_%H-%M-%S")}'
    ...
    budget_hours_by_project = load_budget_hours_by_project(script_dir)
    ...
    if employee_col:
        ...
        project_sparkline_images = create_entity_employee_sparkline_plots(
            ...
            script_dir=script_dir,
            latest_revision=latest_revision,
            budget_hours_by_project=budget_hours_by_project,
        )
